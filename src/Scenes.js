...
class Scenes {
  ...
  collectDataScene() {
    ...
    data.enter((ctx) => {
      ctx.reply(Strings.greet, this.keyboard.mainKeyboard(ctx));
    });
  ...
  physiciansGuide() {
    const getGuideScene = new Scene("physiciansGuide");

    getGuideScene.enter((ctx) => {
      ctx.reply(
        guide.getGuide(),
        this.keyboard.mainKeyboard(ctx)
      );
    });

    getGuideScene.leave((ctx) => {});

    return getGuideScene;
  }
  ...
  getPhone() {
    ...
    getphone.on("message", (ctx) => {
      ...
      if (ctx.message.contact != undefined) {
        msg = ctx.message.contact.phone_number
          .split(" ")
          .reduce((t, s) => t + s);
  ...
  getAreaOfWorksScene() {
    ...
    getareaofwork.action(/[0-9]+/, (ctx) => {
      if (ctx.flow.state.areaOfWork === undefined) {
        ctx.flow.state.areaOfWork = Array(
          Strings.area_of_work_list.length
        ).fill(false);
      }
      ctx.flow.state.areaOfWork[ctx.match[0]] = !ctx.flow.state.areaOfWork[
        ctx.match[0]
      ];
      ctx.editMessageText(
        Strings.area_of_work,
        this.keyboard.areaOfWorkKeyboard(ctx.flow.state.areaOfWork)
      );
    });
  ...
  symptomsScene() {
    ...
    symptomsScene.action(/[0-9]+/, (ctx) => {
      if (ctx.flow.state.symptom === undefined) {
        ctx.flow.state.symptom = Array(Strings.symptoms_list.length).fill(
          false
        );
      }
      ctx.flow.state.symptom[ctx.match[0]] = !ctx.flow.state.symptom[
        ctx.match[0]
      ];
      ctx.editMessageText(
        Strings.symptom_header,
        this.keyboard.symptomKeyboard(ctx.flow.state.symptom)
      );
    });
  ...
  ppesUsedScene() {
    ...
    ppesUsedScene.action(/[0-9]+/, (ctx) => {
      ...
      return ctx.editMessageText(
        "ppe used",
        this.keyboard.ppeKeyboard(ctx.flow.state.ppe)
      );
    });
  ...
  finScene() {
    ...
    fin.enter((ctx) => {
      let data = {
        ...
        AreaofWork: Strings.area_of_work_list.filter((area, i) => {
          return ctx.flow.state.areaOfWork[i];
        }),
        long: ctx.flow.state.long,
        lat: ctx.flow.state.lat,
        symptoms: Strings.symptoms_list.filter((area, i) => {
          return ctx.flow.state.symptom[i];
        }),
        PPEsUsed: Strings.ppe_list.filter((area, i) => {
          return ctx.flow.state.ppe[i];
        }),
  ...
  setGuide() {
    const guideScene = new Scene("setGuide");
    guideScene.enter((ctx) => {
      if (
        admins.isAdmin(
        ...
        ctx.reply(Strings.guide, this.keyboard.cancelKeyboard());
      else {
        ctx.reply(Strings.access_denied, this.keyboard.mainKeyboard(ctx));
        ctx.flow.enter("collectDataScene", ctx.flow.state);
      }
    });

    guideScene.on("message", (ctx) => {
      if (this.URLRegEx.test(ctx.message.text)) {
        guide.setURL(ctx.message.text);
        ctx.reply(Strings.urlSet,this.keyboard.mainKeyboard(ctx));
      } else {
        ctx.reply(Strings.invalidInput);
        ctx.flow.enter("setGuide", ctx.flow.state);
      }
    });

    guideScene.leave((ctx) => {});
  ...
  manageAdmins() {
    ...
    mngAdmins.action(/^@.+$/, (ctx) => {
      ...
      ctx.editMessageReplyMarkup(
        this.keyboard.manageAdminKeyboard(ctx).reply_markup
      );
  ...
  getTelegramHandel() {
    ...
    getTelegramHandel.on("message", (ctx) => {
      if (ctx.message.text == "" || ctx.message.text == undefined) {
        ...
        admins.insert({
          telegramHandel: ctx.message.text,
          name: ctx.flow.state.first_name,
        });
        ctx.flow.enter("mngAdmins", ctx.flow.state);
      }
    });
